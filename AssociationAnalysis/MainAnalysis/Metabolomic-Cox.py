...
cov_f_lst = ['age', 'sex', 'race', 'educ', 'tdi', 'smk', 'alc', 'bmi', 'fastingtime', 'cl_med']
my_formula = "age + C(sex) + C(race) + educ + tdi + C(smk) + C(alc) + bmi + fastingtime + C(cl_med) + x_omics"
...
myout_df.to_csv(dpath + 'Results/Association/'+omics+'_Step'+str(step)+'-adj_cl_med.csv', index=False)
