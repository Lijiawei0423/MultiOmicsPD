...
dpath = '..'
outpath = dpath+'Output/multi_cov/'
...
cov_df = pd.read_csv(dpath+'Covariates.csv', usecols=['eid', 'age','sex','educ','smk','sbp','bmi','TBI'])
cov_f_lst = ['age','sex','educ','smk','sbp','bmi','TBI']
...
for fold_id in fold_id_list:
    ...
    prors_df.to_csv(outpath + str(fold_id) + '.csv', index = False)
