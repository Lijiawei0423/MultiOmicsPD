...
bld_idx_df = pd.read_csv(dpath+'BloodDataIndex.csv',
                         usecols = ['eid', 'Step', 'cv_fold', 'in_cv_fold','ClinicalLab'])
...
pro_f_lst, pro_df = get_sig_omic(dpath, 'ClinicalLab')
...
train_eids = mydf[(mydf['Step'] == 1) & (mydf['ClinicalLab'] == 1)]['eid']
...
prors_df.to_csv(outpath + '/ClinicalLabData/LightGBM_10y'+ '.csv', index = False)
print('finished')