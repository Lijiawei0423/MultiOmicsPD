...
bld_idx_df = pd.read_csv(dpath+'BloodDataIndex.csv',
                         usecols = ['eid', 'Step', 'cv_fold', 'in_cv_fold','Metabolomic'])
bld_dict = pd.read_csv(dpath+'BloodDict.csv',
                       usecols = ['Omics_group', 'Omics_feature', 'Omics_fullname', 'Omics_code'])

tgt_df = pd.read_csv(dpath + 'PD_outcomes.csv', usecols=['eid', 'target_y'])

pro_f_lst, pro_df = get_sig_omic(dpath, 'Metabolomic')
...
train_eids = mydf[(mydf['Step'] == 1) & (mydf['Metabolomic'] == 1)]['eid']
...
prors_df.to_csv(outpath + '/MetabolomicData/LightGBM_10y'+ '.csv', index = False)
print('finished')