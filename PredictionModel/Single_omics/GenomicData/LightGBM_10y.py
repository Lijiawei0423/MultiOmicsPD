...
bld_idx_df = pd.read_csv(dpath+'BloodDataIndex.csv',
                         usecols = ['eid', 'Step', 'cv_fold', 'in_cv_fold','Genomic'])
...
train_eids = mydf[(mydf['Step'] == 1) & (mydf['Genomic'] == 1)]['eid']
...
prors_df.to_csv(outpath + '/GenomicData/LightGBM_10y'+ '.csv', index = False)
